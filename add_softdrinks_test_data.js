...
// Database connection
const pool = new Pool({
  connectionString: process.env.DATABASE_URL || 'postgresql://postgres:password@localhost:5432/stock_nexus'
});
...
async function addTestData() {
  ...
  try {
    ...
    const branchesResult = await client.query('SELECT id, name FROM branches LIMIT 1');
    ...
    await addMovements(client, itemIds[0], userId, [
    ...
    await addMovements(client, itemIds[1], userId, [
    ...
    await addMovements(client, itemIds[2], userId, [
    ...
    await addMovements(client, itemIds[3], userId, [
    ...
    await addMovements(client, itemIds[4], userId, [
    ...
    await addMovements(client, itemIds[5], userId, [
    ...
    await addMovements(client, itemIds[6], userId, [
    ...
    await addMovements(client, itemIds[7], userId, [
      { type: 'in', quantity: 30, days_ago: 7, reason: 'Weekly delivery' },
      { type: 'out', quantity: 25, days_ago: 6, reason: 'Customer sales' },
      { type: 'out', quantity: 20, days_ago: 5, reason: 'Customer sales' },
...
async function addMovements(client, itemId, userId, movements) {
