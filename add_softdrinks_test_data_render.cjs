...
async function addTestDataToRender() {
  try {
    console.log('🍹 Adding Soft Drinks Test Data to Render Database...');
    ...
    // Check if soft drinks items already exist
    const existingItems = await query('SELECT COUNT(*) as count FROM items WHERE category = $1', ['softdrinks']);
    if (existingItems.rows[0].count > 0) {
      console.log(`⚠️ Found ${existingItems.rows[0].count} existing soft drinks items. Skipping item creation.`);
    } else {
      ...
      console.log('✅ Soft drinks test data added successfully to Render!');
    }
    ...
    console.log('\n🎯 Now you can test the Soft Drinks Weekly Comparison Report in your deployed app!');
    
  } catch (error) {
    console.error('❌ Error adding test data to Render:', error);
  }
}
...
// Run the script
addTestDataToRender().catch(console.error);
