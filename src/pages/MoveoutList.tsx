...
const MoveoutList = () => {
  ...
  const generateMoveoutList = async () => {
    ...
    try {
      const moveoutListData = await apiClient.createMoveoutList(itemsWithRequestAmount);
      setGeneratedReport(moveoutListData);
      
      // Refresh historical lists, excluding the newly created report
      const historicalData = await apiClient.getMoveoutLists();
      const filteredData = historicalData?.filter(list => list.id !== moveoutListData.id) || [];
      setHistoricalLists(filteredData);
  ...
  const canManageMoveout = profile?.role === 'manager' || profile?.role === 'assistant_manager';
